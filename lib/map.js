...
function feature_details(data_item){
    ...
    for(var i = 0; i < dataList.length; i++){
        if(dataList[i].OWNER+"."+dataList[i].TABLE_NAME === data_item){    
            ...
            $('#d-location').html('V:\\'+di.OWNER+'\\'+di.OWNER+'.sde');
            $('#d-feature').html(di.TABLE_NAME);
            $('#d-dataset').html(getDataset(di));
            $('#d-type').html(getType(di));
            $('#d-count').html(di.ITEM_COUNT);
            $('#d-modified').html(di.MOD_DATE);
            $('#d-abstract').html(di.ABSTRACT);
            $('#d-purpose').html(di.PURPOSE);
            $('#d-limits').html(di.USE_LIMIT);
            /*if(di.USE_LIMIT !== ''){
                $('#d-limits').html(di.USE_LIMIT);
                $('#use-panel').addClass('alert-warning');
            }else{
                $('#use-panel').removeClass('alert-warning');
                $('#d-limits').html('No use limit or restrictions defined in metadata. Check with data manager.');
            }*/
           // $('#d-limits').html(di.USE_LIMIT);
            populate_field_table(data_item);
        }
    }
}

function populate_field_table(dm){
